...
2. **域名/IP 关联（BPF DNS Parser 映射表）**  
   由于主机环境的 `process_dns` 通常不直接携带 Answer IP（你已验证），TetraBlocker 使用：
...
### 学习阶段（默认 5 分钟）
- TetraBlocker 启动后进入学习窗口（默认 300 秒，可配置）
- 期间它会记录主机上发生过的网络行为：
  - binary（进程二进制路径）
  - proto（tcp/udp）
  - port（目的端口）
...
为了做到“只允许 `curl google.com`，阻断 `curl sina.com`”，TetraBlocker 使用 **同一 hook 多个 selector 的短路匹配** 语义：

- 对每个 binary（或 binary pattern）生成一个 `tb-allowlist-...` 策略：
...
## 优势（为何适合 PoC/演示与受控生产场景）

1. **无需 Kubernetes / 无需 Cilium Network Policy**  
   在传统主机上也能实现“按域名”访问控制体验。

2. **真正精确：binary + domain + ip + port + proto**  
   不是粗粒度“允许某个进程所有外联”，而是可控到具体目的地。
...
3. **低延迟优化与效率改进（相对朴素轮询方案）**  
   ...
   - YAML hash 去抖：内容不变不重复 add，减少 policy churn
   - 声明式 reconcile：配置/allowlist 变化后可幂等更新

4. **企业版 Tetragon 的关键价值被充分放大**
